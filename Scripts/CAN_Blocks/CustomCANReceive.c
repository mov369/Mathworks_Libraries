 ...
 *  -------------------------------------------------------------------------
 *
 * Created: Tue Jun 13 14:42:10 2023
 */

#define S_FUNCTION_LEVEL               2
#define S_FUNCTION_NAME                CustomCANReceive
...
extern void CustomCANReceive_Outputs_wrapper(const real_T *u0,
  CAN_MESSAGE_BUS *y0);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
  ...
  CAN_MESSAGE_BUS _y0BUS;
  slDataTypeAccess *dta = ssGetDataTypeAccess(S);
  const char *bpath = ssGetPath(S);

  /*Copy from Simulink to bus structure*/
  CustomCANReceive_Outputs_wrapper(u0, &_y0BUS);

  /*Copy from bus structure to Simulink*/

  /*Copy from _y0BUS to Simulink for Output port 0*/
  *((uint8_T*)((char *) y0 + busInfo[0].offset)) = (_y0BUS).Extended;
  *((uint8_T*)((char *) y0 + busInfo[1].offset)) = (_y0BUS).Length;
  *((uint8_T*)((char *) y0 + busInfo[2].offset)) = (_y0BUS).Remote;
  *((uint8_T*)((char *) y0 + busInfo[3].offset)) = (_y0BUS).Error;
  *((uint32_T*)((char *) y0 + busInfo[4].offset)) = (_y0BUS).ID;
  *((real_T*)((char *) y0 + busInfo[5].offset)) = (_y0BUS).Timestamp;
  (void) memcpy((char *) y0 + busInfo[6].offset, (_y0BUS).Data, 8*busInfo[6].
                elemSize);
}
